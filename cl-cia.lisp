...
(defmethod print-object ((o user) stream)
	     ...
	     (find-if (lambda (alias)
			(and
			 ...
			 (find name (alias alias) :test #'string-equal)))
		      (aliases user)))
	   (users p)))
(defmethod find-association ((p project) (u user))
  (find-if (lambda (a) (eq (project a) p)) (aliases u)))
(defmethod flagp ((p project) (u user) flag)
  (alexandria:when-let ((association (find-association p u)))
    (find flag (flags association))))
(defmethod set-flag ((p project) (u user) flag)
  (alexandria:when-let ((association (find-association p u)))
    (unless (find flag (flags association))
      (push flag (flags association)))))
(defmethod unset-flag ((p project) (u user) flag)
  (alexandria:when-let ((association (find-association p u)))
    (when (find flag (flags association))
      (setf (flags association) (remove flag (flags association))))))

(defmethod adminp ((p project) (u user)) (flagp p u 'admin))
(defmethod set-admin ((p project) (u user)) (set-flag p u 'admin))
(defmethod unset-admin ((p project) (u user)) (unset-flag p u 'admin))
