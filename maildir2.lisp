...
(defun do-gci-email (header-fields body hooks)
  ...
  (setf body (format nil "~{~a~^ ~}" (mapcar (lambda (x) (string-trim " " x)) (subseq body 0 (position-if (lambda (x) (when (> (length x) 2) (string= (subseq x 0 2) "--"))) body)))))
	(setf id (car (cl-ppcre:all-matches-as-strings "(?<=gci2012/)[0-9]{7}(?=:)" body)))
	(setf name (car (cl-ppcre:all-matches-as-strings ".*(?= has left)" body)))
	(setf task (car (cl-ppcre:all-matches-as-strings "(?<=comment at ).*(?= http://)" body)))
	(setf title (car (cl-ppcre:all-matches-as-strings (concatenate 'string "(?<=" id ":   ).*?(?=  )") body)))
	(setf comment (car (cl-ppcre:all-matches-as-strings (concatenate 'string "(?<=  ).*?(?=   Greet)") body)))
	(setf body (concatenate 'string (ascii-ize "GCI" 3) ":" name " * " id " " task ": " title " - " comment))
      (post-message "#maths22" (truncate-for-irc body 200)))
