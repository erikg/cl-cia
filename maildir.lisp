...
(defun do-gci-email (header-fields body hooks)
  ...
  (let (
  (id (car (cl-ppcre:all-matches-as-strings "(?<=gci2012/)[0-9]*(?=:)" body)))
  (user (car (cl-ppcre:all-matches-as-strings ".*(?= has left)" body)))
  (task (car (cl-ppcre:all-matches-as-strings "(?<=comment at ).*?(?= http)" body)))
  (title (car (cl-ppcre:all-matches-as-strings "(?<=:\s).*" body)))
  (content (car (cl-ppcre:all-matches-as-strings (concatenate "((?<=" title "\s).*" body))))
  (setf body(concatenate 'string "GCI:" user " * " id " "  task ": " title " - " content)))
      (post-message "#brlcad"
			    (truncate-for-irc body)))
